...
function App() {
  ...
  const sendToImageToPrompt = async (imageUrl: string) => {
    try {
      const response = await fetch(assetUrl(imageUrl))
  ...
  const loadHistory = async () => {
    ...
    try {
      const [historyRes, favoritesRes] = await Promise.all([
        ...
        fetch(apiUrl('/api/history/favorites')),
      ])
      if (historyRes.ok) {
  ...
  const loadAvatars = async () => {
    ...
    try {
      const response = await fetch(apiUrl('/api/avatars'))
      if (response.ok) {
        const data = await response.json()
  ...
  const selectAvatar = async (avatar: Avatar) => {
    try {
      ...
      const response = await fetch(assetUrl(avatar.url))
  ...
  const addToFavorites = async (prompt: GeneratedPrompt, name: string) => {
    try {
      const response = await fetch(apiUrl('/api/history/favorites'), {
        method: 'POST',
  ...
  const removeFromFavorites = async (id: string) => {
    try {
      const response = await fetch(apiUrl(`/api/history/favorites/${id}`), { method: 'DELETE' })
      if (response.ok) {
  ...
  const loadVoices = async () => {
    ...
    try {
      const response = await fetch(apiUrl('/api/avatars/voices'))
      if (response.ok) {
        const data = await response.json()
...
sharp focus, detailed skin texture, 8k uhd, high resolution, photorealistic, professional photography`
    ...
    try {
      for (let i = 0; i < avatarCount; i++) {
        ...
        const response = await fetch(apiUrl('/api/avatars/generate'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            prompt,
  ...
  const handleGenerateScript = async () => {
    ...
    try {
      const response = await fetch(apiUrl('/api/avatars/script'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleGenerateTTS = async () => {
    ...
    try {
      const response = await fetch(apiUrl('/api/avatars/tts'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const handleCreateLipsyncVideo = async () => {
    ...
    try {
      const response = await fetch(apiUrl('/api/avatars/lipsync'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          imageUrl: avatarUrl,
  ...
  const pollMachineBatch = async (jobId: string, signal: AbortSignal): Promise<BatchProgress> => {
    ...
    while (!signal.aborted) {
      try {
        const res = await fetch(apiUrl(`/api/generate/progress/${jobId}`), { signal })
  ...
  const handleRunMachine = async (resumeFrom?: MachineStep) => {
    ...
    try {
      ...
      if (startIdx <= 0) {
        ...
        const res = await fetch(apiUrl('/api/prompts/generate'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (startIdx <= 1) {
        ...
        const avatarRes = await fetch(assetUrl(avatarUrl!), { signal })
        ...
        const res = await fetch(apiUrl('/api/generate/batch'), { method: 'POST', body: formData, signal })
      ...
      if (startIdx <= 2) {
        currentStep = 'script'
        setMachineStep(currentStep)
        const res = await fetch(apiUrl('/api/avatars/script'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (startIdx <= 3) {
        currentStep = 'tts'
        setMachineStep(currentStep)
        const res = await fetch(apiUrl('/api/avatars/tts'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (startIdx <= 4) {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
          try {
            const res = await fetch(apiUrl('/api/avatars/lipsync'), {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
  ...
  const handleGenerate = async () => {
    ...
    try {
      response = await fetch(apiUrl('/api/prompts/generate'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const convertTextToPrompt = async (text: string): Promise<GeneratedPrompt | null> => {
    try {
      const response = await fetch(apiUrl('/api/prompts/text-to-json'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleBatchGenerate = async () => {
    ...
    try {
      ...
      response = await fetch(apiUrl('/api/generate/batch'), {
        method: 'POST',
        body: formData,
      })
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const response = await fetch(apiUrl(`/api/generate/progress/${batchProgress.jobId}`))
        if (response.ok) {
          failedPolls = 0
          const data = await response.json()
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const response = await fetch(apiUrl('/api/generate/analyze-image'), {
                          method: 'POST',
                          body: formData,
                        })
                  ...
                  {batchProgress.status === 'completed' && (
                    <div className="mt-4 pt-4 border-t border-gray-800 flex justify-end">
                      <button
                        onClick={async () => {
                          try {
                            const response = await fetch(apiUrl('/api/generate/open-folder'), {
                              method: 'POST',
                              headers: { 'Content-Type': 'application/json' },
                ...
                <div className="bg-gray-900 rounded-lg p-4">
                  ...
                  {(selectedAvatar || generatedAvatarUrls.length > 0) && (
                    <div className="mt-4 p-3 bg-gray-800 rounded-lg">
                      ...
                      <div className="flex items-center gap-3">
                        <img
                          src={assetUrl(generatedAvatarUrls[selectedGeneratedIndex] || selectedAvatar?.url || '')}
                          alt="Selected avatar"
                ...
                <div className="bg-gray-900 rounded-lg p-6">
                  ...
                  {machineAudioUrl && (
                    <div className="mb-6">
                      ...
                      <audio controls src={assetUrl(machineAudioUrl!)} className="w-full" />
                    </div>
                  )}
                  ...
                  {machineVideoUrl && (
                    <div className="mb-6">
                      ...
                      <div className="flex gap-4 items-start">
                        <video controls src={assetUrl(machineVideoUrl!)} className="max-w-sm rounded-lg" />
                        <a
                          href={assetUrl(machineVideoUrl!)}
                          download
